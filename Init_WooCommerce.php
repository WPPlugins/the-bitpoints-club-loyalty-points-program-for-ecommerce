...
function BitPointsClub_WooCommerce_custom_cart_field( $checkout ) {
    ...
    if(BitPointsClub_loggedin()) {
        ...
        if(!is_numeric($Min_Points_Value)) $Min_Points_Value = 0;
...
function BitPointsClub_WooCommerce_order_status_changed($order_id, $old_status, $new_status) {   
    ...
    if(strtoupper($old_status) == strtoupper("pending") && (strtoupper($new_status) == strtoupper("processing") || strtoupper($new_status) == strtoupper("completed"))) {
    ...
    else if(strtoupper($new_status) == strtoupper("failed") || strtoupper($new_status) == strtoupper("cancelled") || strtoupper($new_status) == strtoupper("refunded")) {
        $order = new WC_Order($order_id);
        $fees = $order->get_fees();
        $current_user = wp_get_current_user();
